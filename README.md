...
| Eine Frage | 9,0 ms englisch, 6,0 ms deutsch | 16 ms für drei Fragen in einem Durchlauf |
| Gegen das Original | 6- bis 12-mal schneller als laya in PyTorch | 2 von 1468 Antworten anders als kev in PyTorch, 23 statt 59 ms im Median |
...
Voraussetzung ist ein Mac mit Apple Silicon, macOS 15 und Xcode 16 oder neuer.
...
- **Rechenwerk.** Beide Modelle laufen auf der GPU. Die Neural Engine wird abgelehnt: Die ersten
  laya-Exporte hat sie falsch gerechnet, die jetzigen richtig, aber 40-mal langsamer.
- **Erste Anfrage.** Laden und Vorbereiten dauern ein bis zwei Sekunden. `warmUp()` beim Start
  im Hintergrund aufrufen, dann wartet keine Anfrage darauf.
...
- **kev und Platte.** Ein geladener kev-Export legt im Temp-Verzeichnis bis zu 14 GB ab, bis der
  Prozess endet. Für eine App ist laya die bessere Wahl.
...
`--buckets` hält mehrere kev-Längen und nimmt je Anfrage die kürzeste passende.

## Die Modelle

| Datei | Modell | Eingabelängen | Größe |
|---|---|---|---|
| `Laya-EN-L512-K512-fp16` | laya, english | 128, 256, 512 | 805 MiB |
| `Laya-ML-L1024-K1024-fp16` | laya, multilingual | 128 bis 1024 | 615 MiB |
| `Laya-TD-L1024-K1024-fp16` | laya, typed-decisions | 128 bis 1024 | 805 MiB |
| `Kev06B-Q4-fp16` | kev, bis 4 Fragen und 8 Optionen je Durchlauf | 512 | 1,1 GiB |
| `Kev06B-L256-Q4-fp16` | kev, kurze Anfragen | 256 | 1,1 GiB |
| `Kev06B-L1024-Q4K96-fp16` | kev, bis 96 Optionen | 1024 | 1,1 GiB |

Jedes laya-Paket nimmt mehrere Eingabelängen an, und die Laufzeit rechnet eine Frage in der
...
| kev, Entwicklungssuite über HTTP | 59 ms | **22,7 ms** | 2,6× |

Gemessen auf einem M5 Max. laya im Original läuft als PyTorch auf der CPU, so wie es sich selbst
misst; kev im Original als `kev.serve` in PyTorch auf der GPU, der Port dazu mit `--buckets`. Gegen
den Cloud-Dienst Jev 1.13, der auf kev beruht, kommt der Port auf Banking77 mit 77 Kategorien auf
...
Tests/                    99 Tests gegen Referenzdaten aus den Originalen
...
```bash
swift test                  # 99 Tests, mit Modellen unter Models/ ohne einen übersprungenen
```
